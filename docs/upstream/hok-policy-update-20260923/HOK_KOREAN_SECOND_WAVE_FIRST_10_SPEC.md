...
2026-09-23 결정 비용 변경: 사용자 요청으로 지역 금융 순환·현행 생산 집중 지원도 **정치력 150만 사용하고 민간공장을 점유하지 않도록** 변경했다. 사람과 AI의 가용 민공 수량 요건을 제거하며 기간·효과·공유 대기·취소·반복 구조는 유지한다. 아래 비용표는 이 후속 변경을 반영한다. 2026-09-22의 149 PASS/0 FAIL은 당시 보상 조정의 기록이며 이번 비용 변경의 검증 결과가 아니다.
...
| `yeo_rural_credit_project` | 자율금융 | 전체 건설 속도 +10% | 150 | 0 | 요건 없음 |
| `pak_plant_expansion_project` | 설비확충 | 군수공장 건설 속도 +15% | 150 | 0 | 요건 없음 |
| `pak_current_output_project` | 현행생산 | 군수공장 생산량 +10% | 150 | 0 | 요건 없음 |

네 사업 모두 사람과 AI의 민공 수량 요건이 없고 민공을 점유하지 않는다. 기존 AI 정치력 150 이상 조건과 기본 가중치 0.5를 유지한다. 공산정부 상실·내전·항복·경로 상실 시 취소하며 기존 효과·90일 기간·공통 90일 대기·동시 하나 제한은 유지한다.
...
기존 진단의 149 PASS/0 FAIL은 2026-09-22 보상 조정 전 기록으로 보존한다. 당시 조정 후에도 새 기대값으로 네 정책을 별도 실행하여 149 PASS/0 FAIL을 확인했다. 당시 결과와 한계는 [보상 변경 검증 기록](incidents/2026-09-22-korean-policy-rewards.md)을 따르며 2026-09-23의 비용 변경을 검증한 결과로 재사용하지 않는다. 정상 35일 진행, UI 비용 지불, 연구 보너스 소모, 90일 종료·재착수는 해당 스크립트 검사로 증명하지 않는다.
